"""
V1 + 分割選抜バックテスト
堅実枠(1-4番人気からNN上位3頭) + 発掘枠(5番人気以降からNN上位2頭) = 5頭
"""
...
def select_top5_split(mc, n_pop=3, n_disc=2, cutoff=4):
    """堅実枠(1-cutoff番人気からQMC上位n_pop) + 発掘枠(cutoff+1番人気以降からQMC上位n_disc)"""
...
def run():
    ...
    for year in range(2016, 2026):
        ...
        # V1従来
        t5_v1 = mc.head(5)
        ov_v1, t5f_v1, w_v1, ana_v1 = eval_top5(t5_v1, rd, a3, a1)

        # 分割選抜
        t5_sp = select_top5_split(mc, n_pop=3, n_disc=2, cutoff=4)
        ov_sp, t5f_sp, w_sp, ana_sp = eval_top5(t5_sp, rd, a3, a1)

        print(f'\n{year}年桜花賞:')
        for label, t5, ov in [('V1従来', t5_v1, ov_v1), ('分割選抜', t5_sp, ov_sp)]:
            print(f'  【{label}】 占有{ov}/3')
            for rk, (_, r) in enumerate(t5.iterrows(), 1):
                u = int(r['umaban']); a = rd[rd['umaban']==u]
                af = int(a.iloc[0]['kakutei_jyuni']) if len(a)>0 else 18
                h = '*' if u in a3 else ' '
                nk = int(r['ninki']) if pd.notna(r['ninki']) else 0
                tag = '堅' if nk <= 4 else '発'
                print(f'    {h}{rk}. [{u:2d}] {r["horse_name"]:16s} {nk:2d}人気 odds={r["odds"]:6.1f} [{tag}] ({af}着)')

        print(f'  実際TOP3:')
        for _, r in act.iterrows():
            print(f'    {int(r["kakutei_jyuni"])}着 [{int(r["umaban"]):2d}] {r["horse_name"]:16s} ({int(r["ninki"])}人気)')

        results_v1.append({'year':year, 'win':w_v1, 'overlap':ov_v1, 'avg':np.mean(t5f_v1), 'ana':ana_v1})
        results_sp.append({'year':year, 'win':w_sp, 'overlap':ov_sp, 'avg':np.mean(t5f_sp), 'ana':ana_sp})
        sys.stdout.flush()
    ...
    print(f'  KPI比較')
    print(f'{"="*70}')
    for label, results in [('V1従来', results_v1), ('分割選抜(堅3+発2)', results_sp)]:
        n = len(results)
        ...
        print(f'\n  【{label}】')
        print(f'    的中: {wn}/{n} ({wn/n*100:.0f}%)  占有: {o2}/{n} ({ovr:.0f}%)  TOP5<=9着: {t5ok}/{n}  穴馬: {an}/{n}')
        for r in results:
            m = '◎' if r['win'] else ('○' if r['overlap']>=2 else ('△' if r['overlap']>=1 else '×'))
            a = '穴★' if r['ana'] else '    '
            print(f'      {r["year"]}: {m} {r["overlap"]}/3  avg{r["avg"]:.1f}  {a}')
