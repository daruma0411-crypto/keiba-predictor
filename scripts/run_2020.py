"""2020年桜花賞のみをv9bで予測し、全頭の詳細データを出力"""
...
def main():
    ...
    for rk, (_, r) in enumerate(mc.iterrows(), 1):
        ...
        print(f'  {rk:2d}位 [{u:2d}] {r["horse_name"]:16s} | '
              f'勝率{r["win_prob"]:6.2%} 複勝{r["top3_prob"]:6.2%} E[rank]={r["expected_rank"]:5.2f} odds={r["odds"]:.1f}')
        print(f'       mu={prow["mu"]:.4f} sigma={prow["sigma"]:.4f} | '
              f'タイム偏差={hf["ema_time_zscore"]:.3f} 末脚={hf["ema_agari"]:.3f} '
              f'直線長={hf["long_stretch_avg"]:.1f} 距離差={hf["prev_dist_diff"]:.0f}')
        print(f'       脚質={hf["avg_run_style"]:.2f} 枠={hf["wakuban"]:.0f} '
              f'EMA着順={hf["ema_finish"]:.2f} 重みEMA={hf["weighted_ema_finish"]:.2f} '
              f'賞金log={hf["log_prize_money"]:.2f} 前走class={hf["prev_race_class"]:.0f}')
        print(f'       騎手勝率={hf["jockey_win_rate"]:.3f} 調教師勝率={hf["trainer_win_rate"]:.3f} '
              f'同距離着順={hf["same_dist_finish"]:.2f} 同馬場着順={hf["same_surface_finish"]:.2f} '
              f'勝率={hf["win_rate"]:.3f} 複勝率={hf["top3_rate"]:.3f}')
        print()
