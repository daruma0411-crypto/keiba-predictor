...
def main():
    ...
    if year not in sakura_ids:
        print(f'ERROR: {year}年桜花賞が見つかりません')
    ...
    tr = feat[
        (feat['date'] < rdate) & (feat['is_turf'] == 1) & (feat['past_count'] > 0) & (feat['finish'] > 0)
    ...
    # 詳細出力
    print(f'\n{"="*130}')
    print(f'  {year}年桜花賞 v9b 詳細シミュレーション結果')
    print(f'{"="*130}')
    ...
    for rk, (_, r) in enumerate(mc.iterrows(), 1):
        u = int(r['umaban'])
        hf = rf[rf['umaban'] == u]
        if len(hf) == 0:
            continue
        ...
        rs_v = hf.get('avg_run_style', 2.5) if hasattr(hf, 'get') else hf['avg_run_style']
        rs_str = rs_label.get(round(rs_v*2)/2, f'{rs_v:.1f}')
        ev = r.get('ev_win', 0)
        ls = hf['long_stretch_avg'] if 'long_stretch_avg' in hf.index else np.nan
        ls_str = f'{ls:.1f}' if pd.notna(ls) else 'N/A'

        prow = ps[ps['umaban'] == u].iloc[0]
        print(f'{rk:>2} [{u:2d}] {r["horse_name"]:<16} {r["win_prob"]:5.1%} {r["top3_prob"]:5.1%} '
              f'{r["expected_rank"]:5.1f}  {r["odds"]:5.1f} {ev:5.2f} '
              f'{hf["ema_time_zscore"]:7.3f}  {hf["ema_agari"]:6.3f} {ls_str:>6} {hf["prev_dist_diff"]:5.0f} '
              f'{rs_str:>4} {int(hf["past_count"]):>3} {jk:<10} {af:>3}着')
    ...
    for _, r in mc.iterrows():
        ...
        if rs <= 1.5:
            ...
            label = '差し/追込'
        print(f'  [{u:2d}] {r["horse_name"]:<16} 脚質値={rs:.2f} ({label}) 枠={int(hf["wakuban"])} 斤量={hf["futan"]:.0f}')
    ...
    for rk, (_, r) in enumerate(mc.iterrows(), 1):
        ...
        if len(hf) == 0:
            continue
        hf = hf.iloc[0]
        print(f'  {rk:2d}位 [{u:2d}] {r["horse_name"]:<16} mu={prow["mu"]:.4f} sigma={prow["sigma"]:.4f} | '
              f'win_rate={hf["win_rate"]:.3f} top3_rate={hf["top3_rate"]:.3f} '
              f'jockey_wr={hf["jockey_win_rate"]:.3f} trainer_wr={hf["trainer_win_rate"]:.3f} '
              f'ema_finish={hf["ema_finish"]:.2f} weighted_ema={hf["weighted_ema_finish"]:.2f} '
              f'prize_log={hf["log_prize_money"]:.2f} prev_class={hf["prev_race_class"]:.0f}')
